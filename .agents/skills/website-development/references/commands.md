...
- `pnpm toolkit:mesh <command> [...]`: run the `toolkit` CLI against the `mesh` site from the root.
- `pnpm toolkit:muse <command> [...]`: run the `toolkit` CLI against the `muse` site from the root.
- The root scripts expand to `pnpm exec toolkit --dir websites/mesh ...` and `pnpm exec toolkit --dir websites/muse ...`.
- `toolkit` assumes the shared `content/notes` and `content/posts` layout; use `pnpm toolkit:mesh --help`, `pnpm toolkit:muse --help`, or `pnpm exec toolkit --help` for the command surface and examples.

## Shared imports

- Import shared remark plugins from `@zlliang/markdown/remark`.
- Import shared rehype plugins from `@zlliang/markdown/rehype`.
